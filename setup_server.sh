...
if [ -f "$TOKEN_MMAP_DIR/manifest.json" ]; then
    python -m training.mjson_token_cache info --cache "$TOKEN_MMAP_DIR" | head -40
    ok "token mmap cache 可用: $TOKEN_MMAP_DIR"
elif [ -f "$TOKEN_CACHE_DIR/manifest.json" ]; then
    python -m training.mjson_token_cache info --cache "$TOKEN_CACHE_DIR" | head -40
    ok "token cache 可用: $TOKEN_CACHE_DIR"
    echo "  如需减少上传文件数量/改用 mmap 读取，可转换:"
    echo "    python -m training.mjson_token_cache convert-mmap \\"
    echo "      --source_cache \"$TOKEN_CACHE_DIR\" \\"
    echo "      --cache \"$TOKEN_MMAP_DIR\" \\"
    echo "      --shard_size 200000"
else
...
echo "  python -m training.mjson_token_cache build-mmap \\"
echo "    --source dataset/datasets_years \\"
echo "    --cache dataset/token_mmap_2021-2026 \\"
echo "    --years 2021,2022,2023,2024,2025,2026 \\"
echo "    --num_workers 16 \\"
echo "    --shard_size 200000"
echo ""
echo "  # 已有 .npz token cache 时，可以直接转换成 mmap"
echo "  python -m training.mjson_token_cache convert-mmap \\"
echo "    --source_cache dataset/token_cache_2021-2026 \\"
echo "    --cache dataset/token_mmap_2021-2026 \\"
echo "    --shard_size 200000"
echo ""
...
echo "  DATA_MODE=token_mmap ./run_train.sh"
echo ""
echo "  # 4. 断点续训"
echo "  ./run_resume.sh"
echo ""
